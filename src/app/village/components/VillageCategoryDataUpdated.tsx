...
export const VillageCategoryData: React.FC<VillageCategoryDataProps> = ({
  ...
  const [editingItem, setEditingItem] = useState<
    UnsustainableLandData | IncomeTrackingData | null
  >(null);
  ...
  const openEditModal = (item: UnsustainableLandData | IncomeTrackingData) => {
    ...
    setFormData({
      ...
      year: item.year,
      value: isCategory1
        ? (item as UnsustainableLandData).unsustainableLand
        : (item as IncomeTrackingData).income,
    });
  };
  ...
  if (unsustainableError || incomeError) {
    return (
      <Alert severity="error">
        Failed to load data:{' '}
        {unsustainableError?.message || incomeError?.message}
      </Alert>
    );
  }

  const currentData = isCategory1 ? unsustainableLandData : incomeData;
  const title = isCategory1
    ? '🌱 Monthly Unsustainable Land Clearings'
    ...
    <Box>
      ...
      >
        ...
        <Button
          ...
          disabled={
            addUnsustainableMutation.isPending || addIncomeMutation.isPending
          }
        >
          Add New Data
        </Button>
      ...
      <Grid container spacing={2}>
        {currentData.map(
          (item: UnsustainableLandData | IncomeTrackingData, index: number) => {
            const monthLabel =
              months.find((m) => m.value === item.month)?.label ||
              `Month ${item.month}`;
            const value = isCategory1
              ? (item as UnsustainableLandData).unsustainableLand
              : (item as IncomeTrackingData).income;
                ...
                <Card>
                  <CardContent>
                    ...
                    <Typography variant="body1" color="text.secondary">
                      {isCategory1 ? 'Land Clearings' : 'Income'}: {unit}{' '}
                      {displayValue}
                    </Typography>
                    <Chip
                      label={
                  ...
                  <CardActions>
                    <Button
                      ...
                      disabled={
                        updateUnsustainableMutation.isPending ||
                    ...
                    <Button
                      ...
                      onClick={() =>
                        item.id &&
                        (isCategory1
                          ? handleDeleteUnsustainableData(item.id)
                          : handleDeleteIncomeData(item.id))
                      }
                      disabled={
                        deleteUnsustainableMutation.isPending ||
      ...
      {currentData.length === 0 && (
        <Alert severity="info">
          No {isCategory1 ? 'unsustainable land' : 'income tracking'} data
          recorded yet. Click &quot;Add New Data&quot; to get started.
        </Alert>
      )}
      ...
      >
        <DialogTitle>
          ...
          {isCategory1 ? 'Unsustainable Land' : 'Income'} Data
        </DialogTitle>
        <DialogContent>
        ...
        <DialogActions>
          ...
          <Button
            ...
            variant="contained"
            disabled={
              addUnsustainableMutation.isPending ||
